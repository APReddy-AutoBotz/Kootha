...
export function validatePhysicalEvidenceManifestV1(
  ...
  if (!v.repository || !gitObjectId.test(v.repository.headSha)
  ...
  const dispositionValid = (["pass", "partial", "blocked"] as const).includes(v.disposition as never);
  if (!dispositionValid) issues.push({ path: "disposition", code: "invalid_value" });
  ...
  if (!v.observation || !timestamp(v.observation.startedAt) || !timestamp(v.observation.endedAt)
    ...
    || (v.disposition === "pass" && telemetryCount === 0)) {
    issues.push({ path: "observation", code: "invalid_value" });
  }
  ...
  if (!v.outcomes || !binaryOutcome(v.outcomes.authentication) || !binaryOutcome(v.outcomes.replay)
    ...
    || !binaryOutcome(v.outcomes.freshness) || !binaryOutcome(v.outcomes.health)) {
    issues.push({ path: "outcomes", code: "invalid_value" });
  }

  // Failed/partial/blocked receipts are truthful evidence and remain valid
  // manifests. Only a physical pass must itself satisfy pass-level outcomes.
  if (v.classification === "physical" && v.disposition === "pass" && v.outcomes) {
    const sequencePass = capabilities?.sequenceSupported === false
      ? v.outcomes.sequence === "not_supported"
      : v.outcomes.sequence === "passed";
    const reconnectPass = capabilities?.reconnectSupported === false
      ? v.outcomes.reconnect === "not_supported"
      : v.outcomes.reconnect === "passed";
    if (v.outcomes.authentication !== "passed" || v.outcomes.replay !== "passed"
      || v.outcomes.freshness !== "passed" || v.outcomes.health !== "passed"
      || !sequencePass || !reconnectPass) {
      issues.push({ path: "outcomes", code: "invalid_value" });
    }
  }
