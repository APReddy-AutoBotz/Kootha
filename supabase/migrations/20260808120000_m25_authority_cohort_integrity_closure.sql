-- Advance the current signal to every newer authoritative period or generation.
-- Historical review evidence remains immutable, but never keeps superseded evidence promotable.
...
-- Promotion is defensively bound to the newest authoritative evaluation even if a
-- future worker regression were to leave the current projection behind.
