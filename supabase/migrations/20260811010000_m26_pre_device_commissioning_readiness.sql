...
create table public.physical_pilot_commissioning_receipts (
  ...
  transition_key uuid not null unique, from_state text, to_state text not null, resulting_version bigint not null,
...
declare v_actor uuid; v_row public.physical_pilot_commissioning%rowtype; v_candidate public.m24f_adapter_candidates%rowtype;
 v_manifest public.m24f_adapter_capability_manifests%rowtype; v_from text; v_receipt public.physical_pilot_commissioning_receipts%rowtype;
begin
  ...
  if v_candidate.id is null or v_candidate.decision_status<>'approved_by_ap' or v_candidate.certification_status<>'passed' or v_candidate.manifest_id is distinct from p_manifest_id or v_manifest.id is null or v_manifest.certification_state<>'passed' then raise exception 'Selected adapter is not approved and certified' using errcode='42501'; end if;
  ...
  if v_row.id is not null and v_row.last_transition_key=p_transition_key then
    ...
    if v_receipt.to_state is distinct from p_new_state or v_receipt.reason_code is distinct from p_reason_code
  ...
  if v_row.id is null then
    ...
    v_from:=null;
    insert into public.physical_pilot_commissioning(gps_device_id,selected_candidate_id,selected_manifest_id,state,network_configuration_class,expected_heartbeat_seconds,last_transition_key,created_by_admin,updated_by_admin)
    values(p_device_id,p_candidate_id,p_manifest_id,p_new_state,p_network_configuration_class,p_expected_heartbeat_seconds,p_transition_key,v_actor,v_actor) returning * into v_row;
  else
    ...
    v_from:=v_row.state;
    update public.physical_pilot_commissioning set state=p_new_state,selected_candidate_id=p_candidate_id,selected_manifest_id=p_manifest_id,
  ...
  values(v_row.id,p_transition_key,v_from,p_new_state,v_row.version,p_reason_code,v_actor,jsonb_build_object('contract_version','m26-readiness-v1','device_id',p_device_id,'candidate_id',p_candidate_id,'manifest_id',p_manifest_id,'state',p_new_state,'network_configuration_class',p_network_configuration_class,'expected_heartbeat_seconds',p_expected_heartbeat_seconds::text,'version',v_row.version)) returning * into v_receipt;
...
begin
 ...
 if c.id is null or c.state<>'commissioning' or c.version<>p_expected_version or c.gps_device_id<>p_device_id or c.network_configuration_class is distinct from p_network_configuration_class
...
begin
 ...
 select * into k from public.gps_device_credential_metadata where id=p_credential_id for share;
 if c.id is null or c.state<>'commissioning' or c.version<>p_expected_version or c.gps_device_id<>p_device_id or c.selected_candidate_id<>p_candidate_id or c.selected_manifest_id<>p_manifest_id
  or not exists(select 1 from public.m24f_adapter_candidates a where a.id=p_candidate_id and a.manifest_id=p_manifest_id and a.decision_status='approved_by_ap' and a.certification_status='passed') or m.certification_state<>'passed'
...
begin
 ...
 if c.id is not null then
  ...
  select exists(select 1 from public.physical_pilot_evidence_receipts e where e.commissioning_id=c.id and e.commissioning_version=c.version and e.selected_candidate_id=c.selected_candidate_id
   and e.manifest_id=c.selected_manifest_id and e.gps_device_id=p_device_id and e.device_identity_hash=public.m22_safe_digest(p_device_id::text)
 ...
 elsif a.decision_status<>'approved_by_ap' or a.certification_status<>'passed' or a.manifest_id is distinct from c.selected_manifest_id or m.certification_state<>'passed' then v_stage:='awaiting_adapter_implementation';v_reasons:=array['adapter_not_certified'];
