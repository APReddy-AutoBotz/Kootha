-- Make every new certification run revoke the authority of the run it supersedes,
-- and keep the SQL support vocabulary aligned with the shared evaluator.

create or replace function public.m24f_invalidate_approval_on_certification_v1()
returns trigger language plpgsql security definer set search_path=pg_catalog,public as $$
declare v_previous text;
begin
  select decision_status into v_previous
  from public.m24f_adapter_candidates where id=new.candidate_id for update;
  if v_previous in ('technically_compatible','approved_by_ap') then
    update public.m24f_adapter_candidates
    set decision_status='technically_blocked',updated_by_admin=new.created_by_admin,updated_at=clock_timestamp()
    where id=new.candidate_id;
    insert into public.m24f_candidate_decision_history(
      candidate_id,previous_status,new_status,actor_admin_id,reason,safe_note,manifest_id,certification_run_id
    ) values(
      new.candidate_id,v_previous,'technically_blocked',new.created_by_admin,
      'certification_evidence_superseded','A fresh AP decision is required after a new certification run is recorded.',
      new.manifest_id,new.id
    );
  end if;
  return new;
end $$;

revoke all on function public.m24f_invalidate_approval_on_certification_v1() from public,anon,authenticated;
...
declare v_actor uuid; v_id uuid; v_manifest public.m24f_adapter_capability_manifests%rowtype; v_failed integer; v_latest public.m24f_certification_runs%rowtype;
begin
 ...
 -- A retry of the exact, fully verified run already bound by the current AP
 -- decision is a no-op. Any genuinely new run goes through the insert trigger and
 -- immediately revokes the prior authority before scenario rows can be appended.
 select * into v_latest from public.m24f_certification_runs
 where candidate_id=p_candidate_id order by completed_at desc nulls last,id desc limit 1;
 if v_latest.id is not null and v_latest.manifest_id=v_manifest.id
    and v_latest.adapter_id=trim(p_adapter_id) and v_latest.adapter_version=trim(p_adapter_version)
    and v_latest.certification_level=p_certification_level and v_latest.certification_state=p_certification_state
    and v_latest.scenario_count=p_scenario_count and v_latest.passed_count=p_passed_count
    and v_latest.result_digest=p_result_digest and v_latest.safe_summary=trim(p_safe_summary)
    and public.m24f_assert_persisted_scenario_truth(v_latest.id)
    and exists(select 1 from public.m24f_adapter_candidates c where c.id=p_candidate_id and c.decision_status='approved_by_ap')
    and exists(select 1 from public.m24f_candidate_decision_history h where h.candidate_id=p_candidate_id and h.new_status='approved_by_ap' and h.manifest_id=v_manifest.id and h.certification_run_id=v_latest.id) then
   return v_latest.id;
 end if;
 ...
 values(p_candidate_id,v_manifest.id,v_manifest.adapter_id,v_manifest.adapter_version,p_certification_level,p_certification_state,true,p_scenario_count,p_passed_count,v_failed,p_result_digest,trim(p_safe_summary),clock_timestamp(),v_actor) returning id into v_id;
 ...
 values('admin',v_actor,'m24f_certification_recorded','m24f_certification_run',v_id,
   jsonb_build_object('candidate_id',p_candidate_id,'manifest_id',v_manifest.id,'certification_run_id',v_id,
     'passed_count',p_passed_count,'scenario_count',p_scenario_count,'synthetic',true));
 return v_id;
end $$;
...
create or replace function public.m25_support_level_v1(p_sample_count integer,p_synthetic boolean)
returns text language sql immutable parallel safe set search_path=pg_catalog as $$
  select case
    when p_synthetic then 'synthetic_only'
    when p_sample_count<=0 then 'none'
    when p_sample_count<3 then 'low'
    when p_sample_count<20 then 'moderate'
    else 'strong'
  end
$$;

revoke all on function public.m25_support_level_v1(integer,boolean) from public,anon,authenticated;
grant execute on function public.m25_support_level_v1(integer,boolean) to service_role;
