-- Reconcile the retained root-correction contract with dependency-specific,
-- paginated propagation.  The root marker remains stable for compatibility;
-- each rebuilt consumer also publishes a distinct edge marker.

alter table public.m25_feature_extraction_jobs
  add column if not exists correction_consumer_snapshot_id uuid
    references public.m25_feature_snapshots(id);

create index if not exists m25_jobs_correction_consumer_idx
  on public.m25_feature_extraction_jobs(correction_consumer_snapshot_id,period_end,id)
...
begin
  if old.input_watermark is distinct from new.input_watermark then
    ...
    new.correction_consumer_snapshot_id:=null;
  end if;
  return new;
end $$;

-- The worker body is installed below from the immediately preceding closure,
-- with root and consumer correction identities kept independently.
...
declare
  ...
  v_dependency_snapshot_id uuid; v_consumer_snapshot_id uuid; v_root_snapshot_id uuid; v_dependency_scope text; v_dependency_scope_key_hash text;
  ...
  for j in select * from public.m25_feature_extraction_jobs where state in ('pending','processing') and next_attempt_at <= p_now and attempt_count < 8 and (state='pending' or locked_at < p_now - interval '5 minutes')
      and not exists (
        ...
        where blocker.correction_root_snapshot_id=m25_feature_extraction_jobs.correction_root_snapshot_id
          ...
          and blocker.state in ('pending','processing','failed')
    ...
    begin
      ...
      if j.authoritative_correction_pending then
        ...
        where later.synthetic=j.synthetic and later.period_end>j.period_end
          ...
          and (later.dependency_cause_snapshot_id is distinct from v_dependency_snapshot_id
            or later.correction_consumer_snapshot_id is distinct from v_consumer_snapshot_id);
        ...
        from (select later.id from public.m25_feature_extraction_jobs later
          where later.synthetic=j.synthetic and later.period_end=(select min(next_period.period_end)
              from public.m25_feature_extraction_jobs next_period
              where next_period.synthetic=j.synthetic and next_period.period_end=v_next_period)
            and (later.state in ('processing','completed','failed') or exists (
              select 1 from public.m25_feature_snapshots prior_snapshot
              where prior_snapshot.scope=later.scope and prior_snapshot.scope_key_hash=later.scope_key_hash
                and prior_snapshot.period_start=later.period_start and prior_snapshot.period_end=later.period_end
        ...
        update public.m25_statistical_signals sig set state='insufficient_data',
          evaluation_id=public.m22_safe_digest(concat_ws('|','m25-fallback-invalidated-v2',sig.id::text,v_consumer_snapshot_id::text))
        ...
        update public.m25_feature_extraction_jobs later set
          ...
          correction_consumer_snapshot_id=v_consumer_snapshot_id,correction_root_snapshot_id=v_root_snapshot_id,next_attempt_at=clock_timestamp(),updated_at=clock_timestamp()
      ...
      where id=j.id;
      -- A successful pagination page is not completion of the correction root.
      if v_propagation_remaining then
        update public.m25_feature_extraction_jobs
          set authoritative_correction_pending=true
        where id=j.id;
      end if;
      v_built:=v_built+1;
