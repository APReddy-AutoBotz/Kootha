-- Preserve authoritative correction evidence across dirty concurrent claims and align
-- SQL signal-state hysteresis with the shared deterministic evaluator. This forward-only
-- migration retains the bounded, retry-safe, service-role-only worker contract.

create or replace function public.m25_signal_state_v1(
  p_score numeric,p_opening_threshold numeric,p_clearing_threshold numeric,
  p_previous_state text,p_consecutive_windows integer,p_required_windows integer,p_supported boolean)
returns text language sql immutable parallel safe set search_path=pg_catalog,public as $$
  select case
    when not p_supported then 'insufficient_data'
    when p_consecutive_windows<p_required_windows then
      case when p_score>=p_clearing_threshold then 'watch' else 'normal' end
    when p_score>=p_opening_threshold then 'investigate'
    when p_previous_state='investigate' and p_score>=p_clearing_threshold then 'watch'
    else 'normal' end
$$;
revoke all on function public.m25_signal_state_v1(numeric,numeric,numeric,text,integer,integer,boolean) from public,anon,authenticated;
grant execute on function public.m25_signal_state_v1(numeric,numeric,numeric,text,integer,integer,boolean) to service_role;
    ...
    begin
      ...
      for v_feature in select * from public.m25_feature_values where snapshot_id=s_id and observation_status='observed' order by feature_id loop
        ...
        for v_definition in select * from public.m25_statistical_signal_definitions where metric=v_feature.feature_id order by signal_id loop
          ...
          select sig.state into v_previous_state from public.m25_statistical_signals sig
          where sig.signal_id=v_definition.signal_id and sig.scope_key_hash=j.scope_key_hash and sig.synthetic=j.synthetic
            and (sig.generated_at<j.period_end or (sig.generated_at=j.period_end and sig.source_generation<j.generation));
