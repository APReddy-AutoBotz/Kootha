-- Close current-certification authority and readiness evidence semantics.
-- Historical certification and decision records remain immutable; operational
-- candidate authority is fail-closed when a later certification ceases to pass.

create or replace function public.m24f_invalidate_approval_on_certification_v1()
returns trigger language plpgsql security definer set search_path=pg_catalog,public as $$
declare v_previous text;
begin
  if new.certification_state not in ('failed','expired') then return new; end if;
  select decision_status into v_previous
  from public.m24f_adapter_candidates where id=new.candidate_id for update;
  if v_previous='approved_by_ap' then
    ...
    insert into public.m24f_candidate_decision_history(
      ...
      'current_certification_invalidated','A fresh AP decision is required after current certification ceased to pass.',
...
drop trigger if exists m24f_invalidate_approval_on_certification on public.m24f_certification_runs;
create trigger m24f_invalidate_approval_on_certification
  after insert on public.m24f_certification_runs
  for each row execute function public.m24f_invalidate_approval_on_certification_v1();
