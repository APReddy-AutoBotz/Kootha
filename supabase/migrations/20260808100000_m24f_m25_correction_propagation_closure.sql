-- Close same-batch correction propagation and reconstruct hysteresis from the latest
-- authoritative prior-period evaluation. This forward-only replacement retains every
-- bounded, retry-safe, service-role-only worker contract from the preceding migration.
