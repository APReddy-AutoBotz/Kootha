...
create table public.m25_feature_extraction_jobs (
  ...
  constraint m25_job_exact_dimensions_check check (
    (scope='device_work_day' and gps_device_id is not null and ad_work_day_id is not null and adapter_version is null and device_model is null)
    or (scope='device_day' and gps_device_id is not null and ad_work_day_id is null and adapter_version is null and device_model is null)
    or (scope='device_model_day' and gps_device_id is null and ad_work_day_id is null and adapter_version is null and device_model is not null)
    or (scope='adapter_version_day' and gps_device_id is null and ad_work_day_id is null and adapter_version is not null and device_model is null)
    or (scope='fleet_day' and gps_device_id is null and ad_work_day_id is null and adapter_version is null and device_model is null)
  )
);
...
create table public.m25_feature_values (
  ...
  constraint m25_feature_value_observation_check check (
    (observation_status = 'observed' and sample_count > 0 and coverage_score > 0)
    or (observation_status in ('unavailable','not_observed') and numeric_value = 0 and sample_count = 0 and coverage_score = 0)
  )
);
...
create unique index m25_baseline_version_identity_unique on public.m25_baseline_versions(baseline_version);
...
create table public.m25_signal_review_history (
  ...
  constraint m25_review_text_check check (char_length(reason) between 1 and 160 and char_length(note) between 1 and 500 and public.m24f_is_safe_metadata(reason) and public.m24f_is_safe_metadata(note))
);
...
create table public.m25_readiness_assessments (
  id uuid primary key default gen_random_uuid(),
  source_snapshot_id uuid not null unique references public.m25_feature_snapshots(id) on delete restrict,
    ...
    begin
      ...
      for v_feature in select * from public.m25_feature_values where snapshot_id=s_id and observation_status='observed' order by feature_id loop
        ...
        for v_definition in select * from public.m25_statistical_signal_definitions where metric=v_feature.feature_id order by signal_id loop
          ...
          where public.m25_statistical_signals.promoted_alert_id is null and public.m25_statistical_signals.state not in ('reviewed','suppressed');
      ...
      select s_id,j.period_end,
        ...
        count(*) filter(where fs.scope='device_work_day')::integer,count(distinct r.gps_device_id)::integer,
