-- Bind M24F certification authority to immutable manifest identity and close M25
-- historical statistical dependency and scorer-parity gaps.

alter table public.m24f_certification_runs add column manifest_id uuid references public.m24f_adapter_capability_manifests(id) on delete restrict;
update public.m24f_certification_runs r set manifest_id=c.manifest_id from public.m24f_adapter_candidates c where c.id=r.candidate_id and c.manifest_id is not null;
alter table public.m24f_certification_runs add constraint m24f_passed_run_manifest_bound_check check(certification_state<>'passed' or manifest_id is not null) not valid;
alter table public.m24f_candidate_decision_history add column manifest_id uuid references public.m24f_adapter_capability_manifests(id) on delete restrict;
alter table public.m24f_candidate_decision_history add column certification_run_id uuid references public.m24f_certification_runs(id) on delete restrict;

alter table public.m25_feature_extraction_jobs
  add column authoritative_correction_pending boolean not null default false,
  add column dependency_cause_snapshot_id uuid references public.m25_feature_snapshots(id) on delete restrict;

create or replace function public.m25_mark_authoritative_correction_v1() returns trigger
language plpgsql set search_path=pg_catalog,public as $$
begin
  if old.input_watermark is distinct from new.input_watermark then new.authoritative_correction_pending:=true; end if;
  return new;
end $$;
create trigger m25_mark_authoritative_correction before update of input_watermark on public.m25_feature_extraction_jobs
for each row execute function public.m25_mark_authoritative_correction_v1();
revoke all on function public.m25_mark_authoritative_correction_v1() from public,anon,authenticated;

create or replace function public.m25_robust_score_v1(p_observed numeric,p_median numeric,p_mad numeric,p_p25 numeric,p_p75 numeric,p_direction text)
returns numeric language sql immutable parallel safe set search_path=pg_catalog,public as $$
 with raw as (select case when p_observed is null or p_median is null then null
   when coalesce(p_mad,0)>0 then 0.6745*(p_observed-p_median)/p_mad
   when coalesce(p_p75-p_p25,0)>0 then 0.6745*(p_observed-p_median)/((p_p75-p_p25)/1.349)
   else null end score)
 select case p_direction when 'high_bad' then greatest(0,score) when 'low_bad' then greatest(0,-score) when 'two_sided' then abs(score) else null end from raw
$$;
revoke all on function public.m25_robust_score_v1(numeric,numeric,numeric,numeric,numeric,text) from public,anon,authenticated;
grant execute on function public.m25_robust_score_v1(numeric,numeric,numeric,numeric,numeric,text) to service_role;

create or replace function public.admin_update_m24f_candidate_metadata_v1(
  p_candidate_id uuid,p_manifest_id uuid default null,p_vendor_sandbox_status text default null,
  p_documentation_status text default null,p_data_residency_status text default null,p_commercial_status text default null,
  p_cost_evidence_status text default null,p_compliance_evidence_status text default null,p_blocking_reason text default null,p_safe_notes text default null
) returns uuid language plpgsql security definer set search_path=pg_catalog,public as $$
declare v_actor uuid; v_candidate public.m24f_adapter_candidates%rowtype;
begin
 v_actor:=public.m20a_require_admin();
 select * into v_candidate from public.m24f_adapter_candidates where id=p_candidate_id for update;
 if v_candidate.id is null then raise exception 'M24F candidate not found' using errcode='P0002'; end if;
 if p_manifest_id is not null and p_manifest_id is distinct from v_candidate.manifest_id
    and (v_candidate.decision_status<>'not_assessed' or exists(select 1 from public.m24f_certification_runs r where r.candidate_id=p_candidate_id)) then
   raise exception 'Authorizing manifest identity is frozen; new certification and decision evidence is required' using errcode='55000';
 end if;
 if (p_blocking_reason is not null and (char_length(p_blocking_reason)>500 or not public.m24f_is_safe_metadata(p_blocking_reason)))
    or (p_safe_notes is not null and (char_length(p_safe_notes)>500 or not public.m24f_is_safe_metadata(p_safe_notes))) then
   raise exception 'Invalid bounded M24F metadata' using errcode='22023';
 end if;
 update public.m24f_adapter_candidates set manifest_id=coalesce(p_manifest_id,manifest_id),vendor_sandbox_status=coalesce(p_vendor_sandbox_status,vendor_sandbox_status),documentation_status=coalesce(p_documentation_status,documentation_status),data_residency_status=coalesce(p_data_residency_status,data_residency_status),commercial_status=coalesce(p_commercial_status,commercial_status),cost_evidence_status=coalesce(p_cost_evidence_status,cost_evidence_status),compliance_evidence_status=coalesce(p_compliance_evidence_status,compliance_evidence_status),blocking_reason=case when p_blocking_reason is null then blocking_reason else nullif(trim(p_blocking_reason),'') end,safe_notes=case when p_safe_notes is null then safe_notes else nullif(trim(p_safe_notes),'') end,updated_by_admin=v_actor,updated_at=clock_timestamp() where id=p_candidate_id;
 insert into public.audit_logs(actor_type,actor_id,action,entity_type,entity_id,safe_details) values('admin',v_actor,'m24f_candidate_metadata_updated','m24f_adapter_candidate',p_candidate_id,jsonb_build_object('contract_version','m24f-adapter-v1'));
 return p_candidate_id;
end $$;

create or replace function public.admin_record_m24f_certification_v1(p_candidate_id uuid,p_adapter_id text,p_adapter_version text,p_certification_level text,p_certification_state text,p_scenario_count integer,p_passed_count integer,p_result_digest text,p_safe_summary text)
returns uuid language plpgsql security definer set search_path=pg_catalog,public as $$
declare v_actor uuid; v_id uuid; v_manifest public.m24f_adapter_capability_manifests%rowtype; v_failed integer;
begin
 v_actor:=public.m20a_require_admin();
 select m.* into v_manifest from public.m24f_adapter_candidates c join public.m24f_adapter_capability_manifests m on m.id=c.manifest_id where c.id=p_candidate_id for update of c;
 if v_manifest.id is null or trim(p_adapter_id)<>v_manifest.adapter_id or trim(p_adapter_version)<>v_manifest.adapter_version then raise exception 'Certification identity must match the attached candidate manifest' using errcode='55000'; end if;
 if p_certification_level not in ('manifest_only','synthetic_conformance') then raise exception 'M24F physical and production certification are deferred' using errcode='55000'; end if;
 if p_result_digest is null or p_scenario_count not between 0 and 200 or p_passed_count not between 0 and p_scenario_count or p_result_digest !~ '^[0-9a-f]{64}$' or p_safe_summary is null or char_length(p_safe_summary) not between 1 and 500 or not public.m24f_is_safe_metadata(p_safe_summary) then raise exception 'Invalid M24F certification result' using errcode='22023'; end if;
 if p_certification_state='passed' and (p_certification_level<>'synthetic_conformance' or p_scenario_count<=0 or p_passed_count<>p_scenario_count) then raise exception 'Passed certification requires nonzero successful synthetic conformance evidence' using errcode='22023'; end if;
 v_failed:=p_scenario_count-p_passed_count;
 insert into public.m24f_certification_runs(candidate_id,manifest_id,adapter_id,adapter_version,certification_level,certification_state,synthetic,scenario_count,passed_count,failed_count,result_digest,safe_summary,completed_at,created_by_admin)
 values(p_candidate_id,v_manifest.id,v_manifest.adapter_id,v_manifest.adapter_version,p_certification_level,p_certification_state,true,p_scenario_count,p_passed_count,v_failed,p_result_digest,p_safe_summary,clock_timestamp(),v_actor) returning id into v_id;
...
returns uuid language plpgsql security definer set search_path=pg_catalog,public as $$
declare v_actor uuid; v_previous text; v_manifest_id uuid; v_latest public.m24f_certification_runs%rowtype;
begin
 v_actor:=public.m20a_require_admin();
 ...
 select decision_status,manifest_id into v_previous,v_manifest_id from public.m24f_adapter_candidates where id=p_candidate_id for update;
 ...
 if p_new_status in ('technically_compatible','approved_by_ap') then
   ...
   if v_manifest_id is null or v_latest.id is null or v_latest.manifest_id is distinct from v_manifest_id
      or not exists(select 1 from public.m24f_adapter_capability_manifests m where m.id=v_manifest_id and m.adapter_id=v_latest.adapter_id and m.adapter_version=v_latest.adapter_version)
      or v_latest.certification_level<>'synthetic_conformance' or not v_latest.synthetic or not public.m24f_assert_persisted_scenario_truth(v_latest.id) then
     raise exception 'Current certification must exactly match the frozen candidate manifest' using errcode='55000';
   end if;
 end if;
 ...
 update public.m24f_adapter_candidates set decision_status=p_new_status,updated_by_admin=v_actor,updated_at=clock_timestamp() where id=p_candidate_id;
 insert into public.m24f_candidate_decision_history(candidate_id,previous_status,new_status,actor_admin_id,reason,safe_note,manifest_id,certification_run_id) values(p_candidate_id,v_previous,p_new_status,v_actor,trim(p_reason),trim(p_safe_note),case when p_new_status in ('technically_compatible','approved_by_ap') then v_manifest_id else null end,case when p_new_status in ('technically_compatible','approved_by_ap') then v_latest.id else null end);
 return p_candidate_id;
end $$;
    ...
    begin
      ...
      for v_feature in select * from public.m25_feature_values where snapshot_id=s_id and observation_status='observed' order by feature_id loop
        ...
        for v_definition in select * from public.m25_statistical_signal_definitions where metric=v_feature.feature_id order by signal_id loop
          ...
          v_state:=case when v_baseline.sample_count<v_definition.minimum_baseline_support or v_feature.sample_count<v_definition.minimum_current_support or v_fallback='none' then 'insufficient_data' when v_score>=v_definition.opening_threshold then 'investigate' else 'normal' end;
      ...
      -- requeues each dependent later window once. Dependency requeues never cascade.
