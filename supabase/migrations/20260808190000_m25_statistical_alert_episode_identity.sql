-- Allocate native statistical alert episode identity under the existing cohort lock.
-- Historical alert episodes remain immutable and exact promotion replay stays idempotent.
...
as $$
declare v_actor uuid; s public.m25_statistical_signals%rowtype; v_alert uuid; v_key text; v_episode integer;
begin
  ...
  -- The cohort authority lock serializes max-plus-one allocation with evaluation,
  -- lifecycle, and racing promotion transactions. Historical terminal alerts remain
  -- the immutable authority for the next episode number.
  select coalesce(max(a.episode_number),0)+1 into v_episode
  from public.alerts a
  where a.dedupe_key=v_key and a.source='statistical_signal';
  ...
  values(null,'statistical_signal','warning','new','Statistical signal requires operational review.',clock_timestamp(),'statistical_signal',v_key,v_episode,true,s.generated_at,s.generated_at,1,null,s.synthetic,'Statistical signal review',s.observed_value,null,'count',clock_timestamp(),clock_timestamp(),'m25_statistical_engine') returning id into v_alert;
