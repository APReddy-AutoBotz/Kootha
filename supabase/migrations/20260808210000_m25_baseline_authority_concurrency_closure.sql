    ...
    begin
      ...
      for v_feature in select * from public.m25_feature_values where snapshot_id=s_id and observation_status='observed' order by feature_id loop
        ...
        from public.m25_select_prior_baseline_v1(v_feature.feature_id,v_feature.source_kind,j.scope,j.scope_key_hash,
          j.device_model,j.adapter_version,j.synthetic,j.period_end,8) selected;
        ...
        values(v_baseline_version,v_feature.feature_id,concat(j.scope,':',j.scope_key_hash),j.device_model,j.adapter_version,v_feature.source_kind,j.synthetic,v_fallback,v_baseline_count,v_coverage_count,v_median,v_mad,v_p25,v_p75,v_min,v_max,j.period_start,j.period_end,j.period_end,false,true,j.generation,s_id)
      ...
      if j.authoritative_correction_pending then
        ...
        update public.m25_statistical_signals sig set
          ...
          evaluation_id=public.m22_safe_digest(concat_ws('|','m25-fallback-invalidated-v1',sig.id::text,s_id::text))
        where sig.synthetic=j.synthetic and sig.generated_at>j.period_end
          and exists (
            select 1 from public.m25_feature_extraction_jobs consumer
