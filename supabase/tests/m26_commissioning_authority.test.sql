...
select ok(pg_get_functiondef('public.m26_has_authoritative_conflict_v1(uuid,uuid,uuid,text,text,timestamptz,timestamptz)'::regprocedure) not ilike '%t.credential_id=p_credential_id and t.gps_device_vehicle_link_id=p_vehicle_link_id%','conflict attempts are not discarded because the original receipt used historical credential or link authority');
