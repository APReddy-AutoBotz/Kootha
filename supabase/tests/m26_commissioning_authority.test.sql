...
select has_index('public','telemetry_identity_conflicts','telemetry_identity_conflicts_m26_scope_idx','conflict lookup has a bounded device and observation-time index');
select ok(pg_get_functiondef('public.m26_has_authoritative_conflict_v1(uuid,uuid,uuid,text,text,timestamptz,timestamptz)'::regprocedure) ilike '%t.credential_id=p_credential_id%' and pg_get_functiondef('public.m26_has_authoritative_conflict_v1(uuid,uuid,uuid,text,text,timestamptz,timestamptz)'::regprocedure) ilike '%t.gps_device_vehicle_link_id=p_vehicle_link_id%','conflicts are scoped to the exact credential and link');
select ok(pg_get_functiondef('public.m26_has_authoritative_conflict_v1(uuid,uuid,uuid,text,text,timestamptz,timestamptz)'::regprocedure) ilike '%t.adapter_id=p_adapter_id%' and pg_get_functiondef('public.m26_has_authoritative_conflict_v1(uuid,uuid,uuid,text,text,timestamptz,timestamptz)'::regprocedure) ilike '%t.adapter_version=p_adapter_version%','conflicts are scoped to the exact adapter and version');
select ok(pg_get_functiondef('public.m26_has_authoritative_conflict_v1(uuid,uuid,uuid,text,text,timestamptz,timestamptz)'::regprocedure) ilike '%event_identity_conflict%' and pg_get_functiondef('public.m26_has_authoritative_conflict_v1(uuid,uuid,uuid,text,text,timestamptz,timestamptz)'::regprocedure) ilike '%sequence_replay_invalid%','both authoritative replay conflict classes are checked');
select ok(pg_get_functiondef('public.service_record_physical_pilot_evidence_v1(uuid,uuid,bigint,uuid,uuid,text,text,uuid,text,uuid,uuid,uuid,uuid,text,timestamptz,timestamptz,bigint,boolean,boolean,text,text,boolean,boolean,text,text[],text)'::regprocedure) not ilike '%k.last_verified_at>p_observation_ended_at%','later verification of the same credential does not stale completed evidence');
