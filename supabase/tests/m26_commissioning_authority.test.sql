...
reset role; set local role service_role; select set_config('request.jwt.claim.role','service_role',true);
select lives_ok(format('select public.service_record_physical_pilot_evidence_v1(%L,%L,2,%L,%L,%L,%L,%L,%L,%L,%L,%L,%L,%L,%L,clock_timestamp(),10,true,true,%L,%L,true,true,%L,%L,%L)','26000000-0000-0000-0000-000000000012',:'fixture_commissioning_id',:'fixture_candidate_id',:'fixture_manifest_id',repeat('b',40),'fixture_workflow',:'fixture_device_id',:'fixture_device_digest',:'fixture_installation_id',:'fixture_link_id','26000000-0000-0000-0000-000000000006','26000000-0000-0000-0000-000000000009','physical',:'fixture_network_validated_at','passed','passed','pass','{}',repeat('d',64)),'fully bound physical-pass fixture is persisted');
reset role;
select observation_started_at as fixture_observation_started_at,observation_ended_at as fixture_observation_ended_at from public.physical_pilot_evidence_receipts where id='26000000-0000-0000-0000-000000000012' \gset
set local role service_role; select set_config('request.jwt.claim.role','service_role',true);
select lives_ok(format('select public.service_record_physical_pilot_evidence_v1(%L,%L,2,%L,%L,%L,%L,%L,%L,%L,%L,%L,%L,%L,%L,%L,10,true,true,%L,%L,true,true,%L,%L,%L)','26000000-0000-0000-0000-000000000012',:'fixture_commissioning_id',:'fixture_candidate_id',:'fixture_manifest_id',repeat('b',40),'fixture_workflow',:'fixture_device_id',:'fixture_device_digest',:'fixture_installation_id',:'fixture_link_id','26000000-0000-0000-0000-000000000006','26000000-0000-0000-0000-000000000009','physical',:'fixture_observation_started_at',:'fixture_observation_ended_at','passed','passed','pass','{}',repeat('d',64)),'exact evidence response-loss replay returns frozen receipt');
reset role; set local role authenticated; select set_config('request.jwt.claim.sub','26000000-0000-0000-0000-000000000001',true); select set_config('request.jwt.claim.role','authenticated',true);
select is(public.admin_get_physical_pilot_readiness_v1((select id from public.gps_devices where device_code='M26-FIXTURE'))->>'stage','ready_for_controlled_physical_pilot','physical pass is ready only under all current authorities');
...
select is(public.admin_get_physical_pilot_readiness_v1((select id from public.gps_devices where device_code='M26-FIXTURE'))->>'stage','ready_for_controlled_physical_pilot','canonical degraded GSM remains operational');
